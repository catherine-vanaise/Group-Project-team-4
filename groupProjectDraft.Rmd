...
modelLowSecEducIntUR <- plm(unemployment ~  avRatio + adultEducTer + lowSecEducGdp*upper,
             data = mydata, index = c("country", "year"), model = "random")
phtest(modelLowSecEducIntUF, modelLowSecEducIntUR)
...
Model with only Long Cycle Tertiary Education and Unemployment

```{r}
m1F <- plm(unemployment ~  longCycleTerEducGdp,
             data = mydata, index = c("country", "year"), model = "within")
m1R <- plm(unemployment ~  longCycleTerEducGdp,
             data = mydata, index = c("country", "year"), model = "random")
phtest(m1F, m1R)
...
```{r}
plot(y=m1F$residuals, x=mydata$longCycleTerEducGdp)
```
